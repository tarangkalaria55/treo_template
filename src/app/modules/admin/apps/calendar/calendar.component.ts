...
export class CalendarComponent implements OnInit, AfterViewInit, OnDestroy {
			...
			timeGrid: {
				...
				columnHeaderHtml: (date) => {
					return `<span class="fc-weekday">${moment(date).format("ddd")}</span>
                            <span class="fc-date">${moment(date).format(
															"D"
														)}</span>`;
				},
	...
	private _openEventPanel(calendarEvent): void {
		...
		this._eventPanelTemplatePortal = new TemplatePortal(
			this._eventPanel,
	...
	private _closeEventPanel(): void {
		...
		if (
			this._eventPanelTemplatePortal &&
			this._eventPanelTemplatePortal.isAttached
		) {
			// Detach it
		...
		if (
			this._eventPanelOverlayRef &&
			this._eventPanelOverlayRef.hasAttached()
		) {
			// Detach it
	...
	changeEventPanelMode(
		...
		if (panelMode === "view") {
			this._eventPanelOverlayRef.removePanelClass([
				"panel-mode-add",
				"panel-mode-edit",
			]);
